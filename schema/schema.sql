...
-- drop table if exists fs_exports cascade;
...
-- ) ;
-- 
-- drop table if exists fs_classes cascade;
...
create table os_types (
  ...
  trusted_nfs               boolean not null default false,
  wpkg                      boolean not null default false,
  last_updated              timestamp not null default now()
) ;
...
drop trigger if exists last_updated_trigger on devices;
create trigger last_updated_trigger before insert or update on devices
  for each row execute procedure set_last_updated();

drop trigger if exists last_updated_trigger on surplus_devices;
create trigger last_updated_trigger before insert or update on surplus_devices
  for each row execute procedure set_last_updated();

drop trigger if exists last_updated_trigger on net_zones;
create trigger last_updated_trigger before insert or update on net_zones
  for each row execute procedure set_last_updated();

drop trigger if exists last_updated_trigger on net_vlans;
create trigger last_updated_trigger before insert or update on net_vlans
  for each row execute procedure set_last_updated();
...
drop trigger if exists last_updated_trigger on net_switches;
create trigger last_updated_trigger before insert or update on net_switches
  for each row execute procedure set_last_updated();
...
drop trigger if exists last_updated_trigger on net_ports_net_vlans;
create trigger last_updated_trigger before insert or update on net_ports_net_vlans
  for each row execute procedure set_last_updated();

drop trigger if exists last_updated_trigger on fs_automounts;
create trigger last_updated_trigger before insert or update on fs_automounts
  for each row execute procedure set_last_updated();

drop trigger if exists last_updated_trigger on os_types;
create trigger last_updated_trigger before insert or update on os_types
  for each row execute procedure set_last_updated();
...
drop trigger if exists last_updated_trigger on people;
create trigger last_updated_trigger before insert or update on people
  for each row execute procedure set_last_updated();
...
drop trigger if exists last_updated_trigger on mail_aliases;
create trigger last_updated_trigger before insert or update on mail_aliases
  for each row execute procedure set_last_updated();

drop trigger if exists last_updated_trigger on user_groups;
create trigger last_updated_trigger before insert or update on user_groups
  for each row execute procedure set_last_updated();

drop trigger if exists last_updated_trigger on courses;
create trigger last_updated_trigger before insert or update on courses
  for each row execute procedure set_last_updated();

drop trigger if exists last_updated_trigger on enrollment;
create trigger last_updated_trigger before insert or update on enrollment
  for each row execute procedure set_last_updated();
